...
def autorss(bot):
    ...
    for filename in os.listdir(RSSFEEDSDIR):
        ...
        config = ConfigParser.ConfigParser()
        config.read(configfile)
        feedname = str(config.get("configuration","feedname"))
        url = str(config.get("configuration","url"))
        parentnumber = str(config.get("configuration","parentnumber"))
        childnumber = str(config.get("configuration","childnumber"))
        trimmedname = str(feedname.replace(" ","").lower())
        lastbuilddatabase = str(trimmedname + '_lastbuildcurrent')
