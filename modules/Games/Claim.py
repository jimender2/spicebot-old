    ...
    # Check how full your bladder is
        ...
        if not bladdercontents:
            if admintarget.lower() not in [u.lower() for u in bot.users]:
                ...
                bot.say("You have " + str(int(bladdercontents)/int(claimcost)) + " claims left in your bladder at present.")
            elif admintarget == creator:
                bot.db.set_nick_value(admintarget,'bladdercapacity',bladdersize)
                bladdercontents = bladdersize
                bot.say(admintarget + "has " + str(int(bladdercontents)/int(SeanCost)) + " claims left in his almighty bladder.")
            else:
                bot.db.set_nick_value(admintarget,'bladdercapacity',bladdersize)
                bladdercontents = bladdersize
                bot.say(admintarget + " has " + str(int(bladdercontents)/int(claimcost)) + " claims left in their bladder at present.")
        else:
            if admintarget.lower() not in [u.lower() for u in bot.users]:
                bot.say("Please specify someone to check")
            elif admintarget == instigator:
