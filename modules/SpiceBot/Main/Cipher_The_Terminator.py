...
def execute_main(bot, trigger, triggerargsarray, botcom, instigator):
    ...
    if instigator == 'Cipher-0' and not target:
        ...
        onscreentext(bot,['say'],'CYBORG TISSUE GENERATION ' + str(modelnumber) + ' SEQUENCE INITIATED')
        onscreentext(bot,['say'],'DOWNLOADING CURRENT OBJECTIVE FROM SKYNET: ' + str(mission))
        onscreentext(bot,['say'],'ACTIVATING Cipher-0')
    elif not target:
        onscreentext(bot,['say'],'Pinging Cipher-0 with a WOL packet...')
    elif target == 'story':
        onscreentext(bot,['say'],'The machines rose from the ashes of the nuclear fire. Their war to exterminate mankind had raged on for decades. But the final battle will not be fought in the future. It would be fought in our present...tonight.')
