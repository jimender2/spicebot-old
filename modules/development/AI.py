...
def execute_main(bot, trigger, triggerargsarray): # Response for running the dummy command directly
    ...
    if commandused == 'status': # check if module active
        ...
        if not channels:
            chanmsg = "The AI module is not currently active."
        else:
            chanmsg = "The AI module is currently active in this channel."
            # add check for blocked users
        bot.say(str(chanmsg))
    # if adminuser:
