...
def spicebot_prerun(bot,trigger):
    ...
    if targetdisenable:
        ...
        if jointime < JOINTIMEOUT and not bot.nick.endswith('dev') and not trigger.sender:
            enablestatus = 1
        ...
        elif lasttime < LASTTIMEOUT and not bot.nick.endswith('dev') and not trigger.sender:
            enablestatus = 1
