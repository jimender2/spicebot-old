...
def spicebot_prerun(bot,trigger):
    ...
    if targetdisenable:
        ...
        if jointime < JOINTIMEOUT and not trigger.sender:# and not bot.nick.endswith('dev'):
            enablestatus = 1
        ...
        elif lasttime < LASTTIMEOUT and not trigger.sender:# and not bot.nick.endswith('dev'):
            enablestatus = 1
