...
def bot_command_process(bot, trigger, triggerargsarray):
    ...
    # Command Used
    botcom.command_main = spicemanip.main(triggerargsarray, 1)
...
def bot_command_function_channel(bot, trigger, botcom, instigator):
    ...
    subcommand = spicemanip.main([x for x in botcom.triggerargsarray if x in valid_subcommands], 1) or 'list'
...
def bot_command_function_on(bot, trigger, botcom, instigator):
    ...
    target = spicemanip.main([x for x in botcom.triggerargsarray if x in botcom.users_all], 1) or botcom.instigator
...
def bot_command_function_modules(bot, trigger, botcom, instigator):
    ...
    # Channel
    channeltarget = spicemanip.main([x for x in botcom.triggerargsarray if x.startswith('#')], 1)
    ...
    if subcommand == 'enable' or subcommand == 'disable':
        ...
        module_adjust = spicemanip.main([x for x in botcom.triggerargsarray if x in bot_visible_coms or x == 'all'], 1) or 'no_module'
...
def bot_command_function_block(bot, trigger, botcom, instigator):
    ...
    subcommand = spicemanip.main([x for x in botcom.triggerargsarray if x in valid_subcommands], 1) or 'current'
    ...
    if subcommand == 'current':
        ...
        if bot_blocked_users != []:
            ...
            current_block_list = spicemanip.main(bot_blocked_users, 'list')
    ...
    if subcommand == 'add' or subcommand == 'del':
        ...
        blocknewlisttext = spicemanip.main(blocknewlist, 'list')
...
def bot_command_function_github(bot, trigger, botcom, instigator):
    ...
    main_subcommand = spicemanip.main([x for x in botcom.triggerargsarray if x in valid_main_subcom], 1) or 'show'
    ...
    if main_subcommand == 'block':
        ...
        if subcommand == 'current':
            ...
            if bot_blocked_users_github != []:
                botmessage.append("The following users are currently blocked:")
                current_block_list = spicemanip.main(bot_blocked_users_github, 'list')
                botmessage.append(str(current_block_list))
            else:
...
def bot_command_function_devmode(bot, trigger, botcom, instigator):
    ...
    subcommand = spicemanip.main([x for x in botcom.triggerargsarray if x in valid_subcommands], 1)
    if not subcommand:
...
def bot_command_function_update(bot, trigger, botcom, instigator):
    ...
    if len(targetbots) == 1:
        ...
        targetbotlist = spicemanip.main(targetbots, 'list')
...
def bot_command_function_pip(bot, trigger, botcom, instigator):
    ...
    if not subcom:
        return osd(bot, trigger.sender, 'say', "pip requires a subcommand. Valid options: " + spicemanip.main(pipcoms, 'andlist'))
...
def bot_command_function_debug(bot, trigger, botcom, instigator):
    ...
    botcom = bot_config_directory(bot, botcom)
    targetbot = spicemanip.main([x for x in botcom.triggerargsarray if x in botcom.config_listing], 1) or bot.nick
