...
def bot_dict_use_cases(bot, maincom, dict_from_file, process_list):
    ...
    for mustbe in process_list:
        ...
        if "react_bot" not in dict_from_file[mustbe].keys():
            dict_from_file[mustbe]["react_bot"] = False
        if "react_self" not in dict_from_file[mustbe].keys():
            dict_from_file[mustbe]["react_self"] = False
...
def bot_target_check(bot, botcom, target, target_self):
    ...
    if str(target).lower() in [u.lower() for u in bot.memory["botdict"]["tempvals"]['all_current_users']] not str(botcom.channel_current).startswith('#'):
