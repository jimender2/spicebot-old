...
def dict_command_configs(bot):
    ...
    for directory in filescan:
        ...
        for quick_coms_type in os.listdir(directory):
            if not os.path.isfile(os.path.join(directory, quick_coms_type)) and len(os.listdir(os.path.join(directory, quick_coms_type))) > 0:
                bot.msg("#spicebottest", "     * " + str(quick_coms_type))

                # iterate over files within
                coms_type_file_path = os.path.join(quick_coms_path, quick_coms_type)
                if os.path.exists(coms_type_file_path) and os.path.isfile(coms_type_file_path) and len(os.listdir(coms_type_file_path)) > 0:
